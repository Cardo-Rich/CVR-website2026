...
- Owner accounts / an owner-facing portal home (statements, quotes, maintenance) — owners remain anonymous-token signers.
...
- Delete `src/pages/portal/agreements.astro` (replaced by the SPA Agreements module) and the `/portal → /portal/agreements` redirect in `astro.config.mjs`. Repoint `/portal` (or a new nav entry) to `/admin`.
