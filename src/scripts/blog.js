/* Public hydration for the /blog index and /blog/[slug] article pages. Rewrites
   the cards / article regions from the published CMS (/api/content), so an
   edit published from the admin editor shows before the next deploy. The baked
   HTML stands when the endpoint is unavailable. Skipped for admins — the inline
   layer owns the draft-aware render there. */
import('./content-hydrate.js').then(function (H) {
  ...
  fetch('/api/content', { headers: { Accept: 'application/json' } })
    ...
    .then(function (d) {
      if (!d || !Array.isArray(d.blog) || !d.blog.length) return;
